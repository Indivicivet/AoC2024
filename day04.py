...
assert PHASE_RATE == np.pi / 4, "using hardcoded phase values for part 2 :)"
template_pt2_raw = np.array([[1, 0, 1], [0, np.exp(1j * np.pi / 4), 0], [1j, 0, 1j]])
convs_pt2 = [
    signal.convolve(phased_pt2, template[::traverse_sign, ::traverse_sign])
    for template in [template_pt2_raw, template_pt2_raw.T]
    for traverse_sign in [-1, 1]
]
print(sum(
    np.sum(np.abs(conv) > 5 - 0.001)
